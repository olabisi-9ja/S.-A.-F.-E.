...
// ---------------------------------------------------------------------------
// SOS SAFETY GUARANTEE
// ---------------------------------------------------------------------------
// SOS / panic alerts are ALWAYS highest priority. AI never gates, ranks, or
// delays an emergency alert - a misclassified emergency is a far worse failure
// mode than a misclassified incident report. The admin Socket.io event and the
// push notification fire synchronously inside triggerAlert() before any of this
// runs; the only deferred action is the SMS fallback, sent after a short, FIXED
// window if the alert is still unacknowledged.
//
// An earlier version asked an LLM to rate "urgency" from little more than the
// user's name, GPS and time of day, and used that guess to pick the SMS timeout
// or escalate immediately. That was neither reliable nor defensible and has been
// removed. AI is now used only to add triage detail to incident REPORTS - never
// to emergency ALERTS.
// ---------------------------------------------------------------------------
// Deterministic triage: every SOS is High priority. No classifier input.
function computeAlertTriage() {
  return {
    rating: 'High',
    escalate_immediately: false,
    timeout_ms: ALERT_SMS_FALLBACK_MS,
  };
}
...
export const triggerAlert = async (req, res) => {
  try {
    ...
    // SOS is always highest priority (see computeAlertTriage). The admin socket
    // event and push notification above have already fired. We now schedule the
    // SMS fallback to fire after a short, FIXED window IF the alert is still
    // unacknowledged. AI never gates this.
    const urgency = computeAlertTriage();
    logger.info(
      `🚨 Alert ${alert.id}: treated as High priority. SMS fallback in ${urgency.timeout_ms}ms if unacknowledged. AI does NOT gate SOS.`
    );
      ...
      try {
        ...
        if (freshAlert && !freshAlert.acknowledged) {
          ...
          if (securityPhone) {
            await sendSMS(
              securityPhone,
              `🚨 SAFE ALERT: ${user.full_name} triggered an emergency at GPS: ${latitude.toFixed(6)}, ${longitude.toFixed(6)}. Action required.`
            );
